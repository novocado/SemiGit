...
@WebServlet(
		description = "사용자가 웹에서 *.up 을 했을 경우 이 서블릿이 응답을 해주도록 한다.",
				urlPatterns = { "*.dak" }, 
			      initParams = { 
			            @WebInitParam(name = "propertyConfig", value = "C:/Users/user/git/SemiGit/SemiGit/src/main/webapp/WEB-INF/Command.properties",
			                  description = "*.dak 에 대한 클래스의 매핑파일")
			      })
		// 또는
        // @WebInitParam(name = "propertyConfig", value = "C:\\NCS\\workspace(jsp)\\MyMVC\\src\\main\\webapp\\WEB-INF\\Command.properties"
		//	, description = "*.up 에 대한 클래스의 매핑파일")
		// \ 는 윈도우에서만 작동함,, / 가 리눅스 , 애플 , 윈도우 모든 곳에서 적용된다.
...
public class FrontController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 원래의 리턴타입이 object 이지만, 우리가 정의한대로 , 부모클래스인 AbstractController 를 모두 거쳐가기 떄문에 , 부모클래스로 형변환한 것이다.
		if ( action == null ) {
			...
			try {
				...
				action.execute(request, response); // 다형성 => 자식 클래스에서 정의한 것을 부모 클래스로 받을 수 있음
				...
				if ( ! bool ) { // viewPage 에 명기된 view단 페이지로 forward(dispatcher)를 하겠다는 말이다.
					...
					if ( viewPage != null ) {
						response.sendRedirect(viewPage); // URL 에 WEB-INF 가 들어가서 안된 적이 존재한다.
